...
void export_IO2GTableListener()
{
	class_<IO2GTableListenerWrap, bases<IAddRef>, boost::noncopyable>("IO2GTableListener", no_init)
		.def("_on_added", pure_virtual(&IO2GTableListener::onAdded))
		.def("_on_changed", pure_virtual(&IO2GTableListener::onChanged))
		.def("_on_deleted", pure_virtual(&IO2GTableListener::onDeleted))
		.def("_on_status_changed", pure_virtual(&IO2GTableListener::onStatusChanged))
		;

	class_<TableListener, TableListenerImpl, bases<IO2GTableListener>, boost::noncopyable>("CTableListener", init<>())
		//.def(init<TableListener>())
		.def("_on_added", &TableListenerImpl::onAdded)
		.def("_on_changed", &TableListenerImpl::onChanged)
		.def("_on_deleted", &TableListenerImpl::onDeleted)
		.def("_on_status_changed", &TableListenerImpl::onStatusChanged)
